...
function fetch_factory()
{
    ...
    $ret = [];
    while ($statement->fetch()) $ret[$id] = ["limit" => $limit ,"sum" => $sum ,"last_update" => $update];
    return $ret;
}
...
function fetch_dish()
{
    ...
    $sql = "SELECT D.id ,D.daily_limit ,D.sum ,D.last_update 
        ...
        WHERE D.department_id = DP.id AND DP.factory = F.id AND F.boss_id = U.id AND U.organization_id = ?;";
    $statement = $mysqli->prepare($sql);
    $statement->bind_param('i', $self->org->id);
    $statement->execute();
    $statement->store_result();
